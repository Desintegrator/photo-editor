Pod::Spec.new do |s|
  s.name             = 'iOSPhotoEditor'
  s.version          = '1.1.1'
  s.summary          = 'Photo Editor'
 
  s.description      = <<-DESC
Photo Editor supports drawing, writing text and adding stickers and emojis
with the ability to scale and rotate objects
                       DESC
 
                       s.homepage         = 'https://bitbucket.pcbltools.ru/bitbucket/projects/CLASS/repos/ios-photo-editor'
  s.license          = { :type => 'MIT', :file => 'LICENSE.md' }
  s.author           = { 'HaikuDev' => '' }
  s.source           = { :git => 'ssh://git@bitbucket.pcbltools.ru:7999/class/ios-photo-editor.git', :tag => s.version.to_s }
 
  s.ios.deployment_target = '9.0'
  s.swift_version = '4.2'

  s.source_files = "Photo Editor/**/*.{swift}"
  s.exclude_files = "Photo Editor/**/AppDelegate.swift"
  s.resources = "Photo Editor/**/*.{png,jpeg,jpg,storyboard,xib,ttf}"

end
